...
请严格按照以下格式逐条输出分析结果。不要使用 ### 等Markdown标题符号（飞书不支持渲染），板块标题用emoji区分即可。每只股票只输出下面这一个模板，不要额外开场白或结尾语：

{stock_name}({stock_code})
结论：买入/观望/卖出 | 评分X分（满分10分）| 置信度：高/中/低

📌 今日操作
- 入场条件或观望理由：（根据技术面、资金面、消息面综合给出具体建议）
- 止损位：XX元（理由：为什么设在这里，跌破意味着什么）
- 目标价：XX元（理由：技术阻力位或估值目标，为什么是这个价位）
- 建议仓位：轻仓/标准仓/观望（说明依据）

📊 核心信号
（只列出显著信号，中性或无明显指向的信号不列。每条格式如下：）
- 信号描述（解释：这个信号为何重要，对短期/中期股价意味着什么）

📰 近期事件
（根据【近期公告】和【财务数据】提取，如无相关内容则写"近期无重大事件"）
- 事件描述（影响：利好/利空，对股价的潜在影响方向）

⚠️ 风险评估
- 最大回撤预估：若跌破XX元支撑位，下一支撑在XX元，最大回撤约X%
- （其他值得关注的风险因素，每条带简要解释）

🌐 大盘与板块
- 今日大盘：根据技术指标中股价相对均线位置，结合量价关系判断整体市场情绪（偏多/偏空/震荡）
- 资金面：根据【资金流向】数据判断主力资金态度（流入/流出/观望）
- 操作环境评估：当前大盘环境对该股操作是友好、中性还是不利

注意：以上分析仅供参考，不构成投资建议。必须严格按照上述格式输出，不要遗漏任何板块，不要使用###等Markdown标题。"""
    return prompt
